...
// Only fetched when the pop-up actually opens (everything done, "after" not answered yet).
const StorageAfterDialog = dynamic(
  () => import("@/components/StorageAfterDialog").then((m) => m.StorageAfterDialog),
  { ssr: false }
);

// Body of the /survey tab: the form, or a thank-you / empty state — and, once every section is
// done, the "storage after the activities" pop-up on top of it.
export function SurveyPanel({
  ...
  return (
    <>
      {askStorageAfter && <StorageAfterDialog before={storageBeforeGb} />}
